...
\item Ein \textbf{(s,t)-Schnitt} ist ein Schnitt $\delta\left(W\right)$
...
\section*{Sonderkapitel Lineare Programmierung}

\subsection*{Formulierung des primalen Programms}

Gesucht wird das minimum/maximum einer lineare Funktion mit lineare
Randbedingungen.

Die Randbedingungen werden als Un- oder Gleichheitsbedingung formuliert

$\begin{aligned}min\; & x_{1} & + & x_{2}\\
 & x_{1} & + & 2x_{2} & \leq & 4\\
 & x_{1} & + & 2x_{2} & \leq & 12\\
- & x_{1} & + & x_{2} & \leq & 1\\
 & x_{1} &  &  & \geq & 0\\
...
\subsection*{Dualität von lineare Programmen}

Lineare Programme können als Maximierungs- oder Minimierungsproblem
formuliert werden.
\[
\begin{aligned}\max(c^{T}x) &  & min(y^{T}b)\\
\begin{aligned}Ax & \leq & b\\
x & \geq & 0
\end{aligned}
 & \qquad\longleftrightarrow\qquad & \begin{aligned}y^{T}A & \geq & c^{T}\\
y & \geq & 0
\end{aligned}
\end{aligned}
\]
...
\[
\begin{aligned}\begin{aligned}c_{1}x_{1} & + & c_{2}x_{2} & + & \cdots & + & c_{n}x_{n} & = & \mathbf{\max}\mathbf{(c}^{T}\mathbf{x)}\\
\\
...
\\
x_{1}\geq0 & , & x_{2}\geq0 & , & \cdots &  & x_{n}\geq0
\end{aligned}
 & \begin{aligned}\qquad\;\qquad\longleftrightarrow\qquad\;\qquad\quad\quad\\
\quad min/max\;Transformierung\quad
\end{aligned}
 & \begin{aligned}y_{1}b_{1} & + & y_{2}b{}_{2} & + & \cdots & + & y_{m}b_{m} & = & \mathbf{\min}\mathbf{(y^{T}}\mathbf{b)}\\
\\
y_{1}a_{11} & + & y_{2}a_{21} & + & \cdots & + & y_{m}a_{m1} & \geq & c_{1}\\
y_{1}a_{12} & + & y_{2}a_{22} & + & \cdots & + & y_{m}a_{m2} & \geq & c_{2}\\
 &  &  &  &  &  &  & \vdots\\
y_{1}a_{1n} & + & y_{2}a_{2n} & + & \cdots & + & y_{m}a_{mn} & \geq & c_{n}\\
\\
y_{1}\geq0 & , & y_{2}\geq0 & , & \cdots &  & y_{m}\geq0
\end{aligned}
\end{aligned}
\]


\subsubsection*{Dualisierungsregeln}

\begin{tabular}{|c|c|c|}
\hline 
 & Primales Programm & Duales Programm\tabularnewline
\hline 
\hline 
Variablen & $x_{1},\ldots,x_{n}$ & $y_{1},\ldots,y_{m}$\tabularnewline
\hline 
Matrix & $A$ & $A^{T}$\tabularnewline
\hline 
Rechte Seite & $b$ & $c$\tabularnewline
\hline 
Zielfunktion & i-te Nebenbedingung hat $\begin{aligned}\leq\\
\geq\\
=
\end{aligned}
$ & $\begin{aligned}y_{i}\geq0\\
y_{i}\leq0\\
y_{i}\in\mathbb{K}
\end{aligned}
$\tabularnewline
\hline 
Nebenbedingungen & $\begin{aligned}x_{j}\geq0\\
x_{j}\leq0\\
x_{j}\in\mathbb{K}
\end{aligned}
$ & j-te Nebenbedingung hat $\begin{aligned}\geq\\
\leq\\
=
\end{aligned}
$\tabularnewline
...
\begin{enumerate}
\item Berechne Zeilenminima $u_{i}=\min\left\{ \bar{c}_{ij}|j=1,\ldots,n\right\} $,
für alle $i=1,\ldots,n$.
\item Setze $\overline{c}_{ij}=c_{ij}-u_{i}$, für alle $i=1,\ldots,n$,
$j=1,\ldots,n$.
\item Berechne Spaltenminima $v_{j}=\min\left\{ \bar{c}_{ij}|i=1,\ldots,n\right\} $,
